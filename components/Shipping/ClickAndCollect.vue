...
const getPickupDate = async (facilityId) => {
  try {
    ...
    const {data} = await useFetch(config.public.LEGACY_URL + '/Ajax/GetPickupDate.rails?facilityId=' + facilityId, {
      credentials: 'include',
      method: 'GET',
...
<template>
  <div class="click-and-collect" show-address="true">
    ...
    <div class="click-and-collect__selected-store">
      <div class="click-and-collect__options">
        <div class="input__group radio " :class="{ 'disabled' : !has2HourPickup}">
          <input
            ...
            :disabled="!has2HourPickup"> <label
            for="pickup_in_2_hours">Recogida express: En 2 horas*  {{!has2HourPickup ? '(no disponible)' : ''}}</label>
        </div>
        <div class="input__group radio">
          <input id="pickup_here"
                 type="radio"
                 v-model="selectedPickupOption"
          ...
          <label v-if="!loadingPickupDate"  for="pickup_here">Recógelo {{isEntre( props.shippingMethod.eta )}} <strong>{{ formatDeliveryDateRange( props.shippingMethod.eta ) }}</strong>.
          Te avisaremos cuando esté preparado.</label>
        </div>
      </div>
      <div class="click-and-collect__notice"><p>* Recogida express
        en 2 horas desde apertura de la tienda. Consulta los horarios de las tiendas
        <a href="https://www.motomundi.cl/tiendas/" target="_blank">aquí</a>
      </p>
        <p style="display: none;">* Descuento en la próxima compra no válido para recogidas express</p></div>
    </div>
    <div class="error">
