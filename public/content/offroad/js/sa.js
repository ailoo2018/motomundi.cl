window.sib.client_key !== '' && (function () {
    ...
    function sr(o) {
        var s = [];
        for (var p in o) {
            Object.prototype.hasOwnProperty.call(o, p) && (s.push(encodeURIComponent(p) + "=" + encodeURIComponent(o[p])));
        }
        return s.join("&");
    }

    function br(d) {
        var td = {key: window.sib.client_key, cuid: Helper.cookie.get('sib_cuid'), ma_url: window.location.href};
    ...
    hl.prototype.cookie = {
        get: function (cn) {
            var name = cn + "=", dc = document.cookie, ca = dc.split(';');
            ...
            var cookie_value = n + "=" + v + ";domain= " + window.location.hostname + ";expires=" + cd.toGMTString() + ";path=/";
    ...
    hl.prototype.send = function (d, cb) {
        var u = config.url + '?' + sr(d), x = new XMLHttpRequest();
        x.open("GET", u, !0);
        x.send();
        return cb && cb(null);
    };
    hl.prototype.post = function (d, cb) {
        var u = config.url, x = new XMLHttpRequest();
        x.open("POST", u, !0);
    ...
    hl.prototype.postReq = function (url, b, h, cb) {
        var u = url, x = new XMLHttpRequest();
        x.open("POST", u, !0);
        setReqHeaders(x, h);
            ...
            (function (v, td) {
                if (v) {
                    ...
                    var jsClosest = Helper.closest(v, function (el) {
                        return el && typeof el.tagName != "undefined" && el.tagName.toLowerCase() == 'a';
                    });
                    if (jsClosest && href) {
                        v.addEventListener("click", function (e) {
                            ...
                            Helper.send(td, function (err) {
                                cb && cb(err);
                                (v.getAttribute('target') !== '_blank') ? setTimeout(function () {
                                    window.location.href = href;
                                }, 1000) : window.open(href);
                            });
                        });
                    }
                }
    ...
    sib.prototype.page = function (n, d, cb) {
        ...
        d = br(d);
        d.sib_type = 'page', d.ma_title = d.title || d.ma_title || document.title, d.sib_name = n || d.ma_title, d.ma_referrer = d.referrer || d.ma_referrer || document.referrer, d.ma_path = d.path || d.ma_path || window.location.pathname;
...
/*
        Helper.postReq(request.url, request.reqBody, request.headers, function (err) {
            return cb && cb(err);
        });
*/
    };
...
*/
    ...
    function load_cm(c) {
        ...
        if (cuid && contact_email && config.custom_url_parsing == "true") {
            const payload = {visitor_id: cuid, email_id: contact_email};
            const session_url = config.base_url + '/visitor/' + window.sib.client_key, x = new XMLHttpRequest();
            ...
            iframe.id = "cm_iframe", iframe.height = "0", iframe.width = "0", iframe.style.display = "none", iframe.style.visibility = "hidden", iframe.src = config.if_url + "&i=0";
