...
jQuery(document).ready(function(){
		jQuery(function(){
	    jQuery('.form-language, .header .block-currency, .toolbar .sort-by, .toolbar .limiter, .block-order-return .return-select').jqTransform({imgPath:'/Content/css/images/") ?>'});
		});
		jQuery(function()
{
});
...
jQuery(document).ready(function() {
  ...
  if(jQuery(".my-account .page-title, .tag-list-index .page-title, .tag-product-list .page-title").length){jQuery('.sidebar').addClass('sidebar-indent'); };
  
		jQuery('.header .links a ').not('.active').hover(function(){
		   jQuery(this).stop(true,false).animate({color:"#fff"}, {duration: 300});
		  },function(){
		   jQuery(this).stop(true,false).animate({color:"#F4C0A1"}, {duration: 300});
		 });
	jQuery('.footer ul li a').hover(function(){
	   jQuery(this).stop(true,false).animate({color:"#fff"}, {duration: 300});
	  },function(){
	   jQuery(this).stop(true,false).animate({color:"#979797"}, {duration: 300});
	 });
