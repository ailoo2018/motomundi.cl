<script setup lang="ts">
</script>
<template>
  <!-- marcas -->
  <VMenu
    open-on-hover
    :close-on-content-click="false"
    attach="#menu-anchor"
    location="bottom"
    class="mega-menu-wrapper "
    width="100%"
    content-class="centered-mega-menu"
  >
    <template #activator="{ props }">
      <li
        class="l0 c3 main-nav"
        v-bind="props"
      >
        <a>
          <span>Marcas</span>
        </a>
      </li>
    </template>
    <ul class="u1 main-nav">
      <li class="l1 c0 primary subCategories">
        <div
          class="submenu"
          style="padding:20px 0;"
        >
          <div class="container">
            <div class="row">
              <div class="col s12 submenu__container">
                <div class="submenu__main-content submenu__no-border">
                  <div class="submenu__header">
                    <span class="h2">
                      <span>Marcas</span>
                    </span>
                  </div>
                  <div
                    id="13"
                    class="submenu__content submenu__outstanding"
                    parent-menu-title="Marcas"
                    icon="/assets/menu/categories/134-marcas.svg"
                    order="7"
                    link=""
                    identifier="13"
                  >
                    <div class="submenu__outstanding-showcase brands">
                      <a
                        href="/agv"
                        class="mtc-link"
                        title="AGV"
                      ><span lazy="true"><img
                        src="https://cf-cdn.motocard.com/cdn-cgi/image/w=80,h=41,q=91,fit=cover,f=auto/brands/outstanding/brand-82.png"
                        class="cdn-img v-lazy-image v-lazy-image-loaded"
                        alt="AGV"
                        width="80"
                        height="41"
                      > </span>
                        <div class="submenu__outstanding-icon" />
                      </a>
                      <a
                        href="/alpinestars"
                        class="mtc-link"
                        title="ALPINESTARS"
                      ><span lazy="true"><img
                        src="https://cf-cdn.motocard.com/cdn-cgi/image/w=80,h=41,q=91,fit=cover,f=auto/brands/outstanding/brand-77.png"

                        class="cdn-img v-lazy-image v-lazy-image-loaded"
                        alt="ALPINESTARS"
                        width="80"
                        height="41"
                      > </span>
                        <div class="submenu__outstanding-icon" />
                      </a>
                      <a
                        href="/arai"
                        class="mtc-link"
                        title="ARAI"
                      ><span lazy="true"><img
                        src="https://cf-cdn.motocard.com/cdn-cgi/image/w=160,h=82,q=91,fit=cover,f=auto/brands/outstanding/brand-83.png"

                        class="cdn-img v-lazy-image v-lazy-image-loaded"
                        alt="ARAI"
                        width="80"
                        height="41"
                      > </span>
                        <div class="submenu__outstanding-icon" />
                      </a>

                      <a

                        href="/dainese"
                        class="mtc-link"
                        title="DAINESE"
                      ><span lazy="true"><img
                        src="https://cf-cdn.motocard.com/cdn-cgi/image/w=160,h=82,q=91,fit=cover,f=auto/brands/outstanding/brand-79.png"
                        class="cdn-img v-lazy-image v-lazy-image-loaded"
                        alt="DAINESE"
                        width="80"
                        height="41"
                      > </span>
                        <div class="submenu__outstanding-icon" />
                      </a>
                      <a

                        href="/givi"
                        class="mtc-link"
                        title="GIVI"
                      >
                        <span lazy="true"><img
                          src="https://www.motomundi.cl/Content/assets/brands/brand-86.png"
                          class="cdn-img v-lazy-image v-lazy-image-loaded"
                          alt="GIVI"
                          width="80"
                          height="41"
                        >
                        </span>
                        <div class="submenu__outstanding-icon" />
                      </a>
                      <a
                        href="/hjc"
                        class="mtc-link"
                        title="HJC"
                      ><span lazy="true"><img
                        src="/content/assets/brands/brand-293.png"
                        class="cdn-img v-lazy-image v-lazy-image-loaded"
                        alt="HJC"
                        width="80"
                        height="41"
                      > </span>
                        <div class="submenu__outstanding-icon" />
                      </a>

                      <a
                        href="/shark"
                        class="mtc-link"
                        title="SHARK"
                      ><span lazy="true"><img
                        src="https://cf-cdn.motocard.com/cdn-cgi/image/w=160,h=82,q=91,fit=cover,f=auto/brands/outstanding/brand-106.png"

                        class="cdn-img v-lazy-image v-lazy-image-loaded"
                        alt="SHARK"
                        width="80"
                        height="41"
                      > </span>
                        <div class="submenu__outstanding-icon" />
                      </a>
                      <a
                        href="/shoei"
                        class="mtc-link"
                        title="SHOEI"
                      ><span lazy="true"><img
                        src="https://cf-cdn.motocard.com/cdn-cgi/image/w=160,h=82,q=91,fit=cover,f=auto/brands/outstanding/brand-78.png"

                        class="cdn-img v-lazy-image v-lazy-image-loaded"
                        alt="SHOEI"
                        width="80"
                        height="41"
                      > </span>
                        <div class="submenu__outstanding-icon" />
                      </a>
                    </div>
                    <div class="submenu__divider">
                      Otras marcas
                    </div>
                    <div class="submenu__outstanding-list">
                      <ul class="">
                        <li>
                          <a
                            href="/abus"
                            class="mtc-link"
                            title="ABUS"
                          >
                            ABUS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/age-of-glory"
                            class="mtc-link"
                            title="AGE OF GLORY"
                          >
                            AGE OF GLORY
                          </a>
                        </li>
                        <li>
                          <a
                            href="/cascos-agv"
                            class="mtc-link"
                            title="AGV"
                          >
                            AGV
                          </a>
                        </li>
                        <li>
                          <a
                            href="/airhawk-seats"
                            class="mtc-link"
                            title="AIRHAWK SEATS"
                          >
                            AIRHAWK SEATS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/airobag"
                            class="mtc-link"
                            title="AIROBAG"
                          >
                            AIROBAG
                          </a>
                        </li>
                        <li>
                          <a
                            href="/alpinestars"
                            class="mtc-link"
                            title="ALPINESTARS"
                          >
                            ALPINESTARS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/cascos-arai"
                            class="mtc-link"
                            title="ARAI"
                          >
                            ARAI
                          </a>
                        </li>
                        <li>
                          <a
                            href="/armadillo"
                            class="mtc-link"
                            title="ARMADILLO"
                          >
                            ARMADILLO
                          </a>
                        </li>
                        <li>
                          <a
                            href="/asmax"
                            class="mtc-link"
                            title="ASMAX"
                          >
                            ASMAX
                          </a>
                        </li>
                        <li>
                          <a
                            href="/axxis"
                            class="mtc-link"
                            title="AXXIS"
                          >
                            AXXIS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/bell"
                            class="mtc-link"
                            title="BELL"
                          >
                            BELL
                          </a>
                        </li>
                        <li>
                          <a
                            href="/bycity"
                            class="mtc-link"
                            title="BYCITY"
                          >
                            BYCITY
                          </a>
                        </li>
                        <li>
                          <a
                            href="/barkbusters"
                            class="mtc-link"
                            title="BARKBUSTERS"
                          >
                            BARKBUSTERS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/bridgestone"
                            class="mtc-link"
                            title="BRIDGESTONE"
                          >
                            BRIDGESTONE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/caberg"
                            class="mtc-link"
                            title="CABERG"
                          >
                            CABERG
                          </a>
                        </li>
                        <li>
                          <a
                            href="/intercomunicadores-cardo"
                            class="mtc-link"
                            title="CARDO"
                          >
                            CARDO
                          </a>
                        </li>
                        <li>
                          <a
                            href="/castrol"
                            class="mtc-link"
                            title="CASTROL"
                          >
                            CASTROL
                          </a>
                        </li>
                        <li>
                          <a
                            href="/chigee"
                            class="mtc-link"
                            title="CHIGEE"
                          >
                            CHIGEE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/clover"
                            class="mtc-link"
                            title="CLOVER"
                          >
                            CLOVER
                          </a>
                        </li>
                        <li>
                          <a
                            href="/crampbuster"
                            class="mtc-link"
                            title="CRAMPBUSTER"
                          >
                            CRAMPBUSTER
                          </a>
                        </li>
                        <li>
                          <a
                            href="/cucyma"
                            class="mtc-link"
                            title="CUCYMA"
                          >
                            CUCYMA
                          </a>
                        </li>
                        <li>
                          <a
                            href="/dainese"
                            class="mtc-link"
                            title="DAINESE"
                          >
                            DAINESE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/elf"
                            class="mtc-link"
                            title="ELF"
                          >
                            ELF
                          </a>
                        </li>
                        <li>
                          <a
                            href="/fhorse"
                            class="mtc-link"
                            title="FHORSE"
                          >
                            FHORSE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/fuel-motorcycles"
                            class="mtc-link"
                            title="FUEL MOTORCYCLES"
                          >
                            FUEL MOTORCYCLES
                          </a>
                        </li>
                        <li>
                          <a
                            href="/falco"
                            class="mtc-link"
                            title="FALCO"
                          >
                            FALCO
                          </a>
                        </li>
                        <li>
                          <a
                            href="/fasthouse"
                            class="mtc-link"
                            title="FASTHOUSE"
                          >
                            FASTHOUSE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/five"
                            class="mtc-link"
                            title="FIVE"
                          >
                            FIVE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/fogcity"
                            class="mtc-link"
                            title="FOGCITY"
                          >
                            FOGCITY
                          </a>
                        </li>
                        <li>
                          <a
                            href="/forma"
                            class="mtc-link"
                            title="FORMA"
                          >
                            FORMA
                          </a>
                        </li>
                        <li>
                          <a
                            href="/fox"
                            class="mtc-link"
                            title="FOX"
                          >
                            FOX
                          </a>
                        </li>
                        <li>
                          <a
                            href="/freedconn"
                            class="mtc-link"
                            title="FREEDCONN"
                          >
                            FREEDCONN
                          </a>
                        </li>
                        <li>
                          <a
                            href="/generico"
                            class="mtc-link"
                            title="GENERICO"
                          >
                            GENERICO
                          </a>
                        </li>
                        <li>
                          <a
                            href="/givi"
                            class="mtc-link"
                            title="GIVI"
                          >
                            GIVI
                          </a>
                        </li>
                        <li>
                          <a
                            href="/go-rigo-go"
                            class="mtc-link"
                            title="GO RIGO GO!"
                          >
                            GO RIGO GO!
                          </a>
                        </li>
                        <li>
                          <a
                            href="/hjc"
                            class="mtc-link"
                            title="HJC"
                          >
                            HJC
                          </a>
                        </li>
                        <li>
                          <a
                            href="/helite"
                            class="mtc-link"
                            title="HELITE"
                          >
                            HELITE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/ipone"
                            class="mtc-link"
                            title="IPONE"
                          >
                            IPONE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/interphone"
                            class="mtc-link"
                            title="INTERPHONE"
                          >
                            INTERPHONE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/john-doe"
                            class="mtc-link"
                            title="JOHN DOE"
                          >
                            JOHN DOE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/just1"
                            class="mtc-link"
                            title="JUST1"
                          >
                            JUST1
                          </a>
                        </li>
                        <li>
                          <a
                            href="/kappa"
                            class="mtc-link"
                            title="KAPPA"
                          >
                            KAPPA
                          </a>
                        </li>
                        <li>
                          <a
                            href="/komine"
                            class="mtc-link"
                            title="KOMINE"
                          >
                            KOMINE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/kore"
                            class="mtc-link"
                            title="KORE"
                          >
                            KORE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/kovix"
                            class="mtc-link"
                            title="KOVIX"
                          >
                            KOVIX
                          </a>
                        </li>
                        <li>
                          <a
                            href="/leatt"
                            class="mtc-link"
                            title="LEATT"
                          >
                            LEATT
                          </a>
                        </li>
                        <li>
                          <a
                            href="/liqui-moly"
                            class="mtc-link"
                            title="LIQUI-MOLY"
                          >
                            LIQUI-MOLY
                          </a>
                        </li>
                        <li>
                          <a
                            href="/mastech"
                            class="mtc-link"
                            title="MASTECH"
                          >
                            MASTECH
                          </a>
                        </li>
                        <li>
                          <a
                            href="/metzeler"
                            class="mtc-link"
                            title="METZELER"
                          >
                            METZELER
                          </a>
                        </li>
                        <li>
                          <a
                            href="/motoaccessories"
                            class="mtc-link"
                            title="MOTOACCESSORIES"
                          >
                            MOTOACCESSORIES
                          </a>
                        </li>
                        <li>
                          <a
                            href="/baterias-motobatt"
                            class="mtc-link"
                            title="MOTOBATT"
                          >
                            MOTOBATT
                          </a>
                        </li>
                        <li>
                          <a
                            href="/motomundi"
                            class="mtc-link"
                            title="MOTOMUNDI"
                          >
                            MOTOMUNDI
                          </a>
                        </li>
                        <li>
                          <a
                            href="/muc-off"
                            class="mtc-link"
                            title="MUC-OFF"
                          >
                            MUC-OFF
                          </a>
                        </li>
                        <li>
                          <a
                            href="/cascos-nexx"
                            class="mtc-link"
                            title="NEXX"
                          >
                            NEXX
                          </a>
                        </li>
                        <li>
                          <a
                            href="/octane"
                            class="mtc-link"
                            title="OCTANE"
                          >
                            OCTANE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/pinlock"
                            class="mtc-link"
                            title="PINLOCK"
                          >
                            PINLOCK
                          </a>
                        </li>
                        <li>
                          <a
                            href="/puig"
                            class="mtc-link"
                            title="PUIG"
                          >
                            PUIG
                          </a>
                        </li>
                        <li>
                          <a
                            href="/rg"
                            class="mtc-link"
                            title="R&amp;G"
                          >
                            R&amp;G
                          </a>
                        </li>
                        <li>
                          <a
                            href="/ram-mounts"
                            class="mtc-link"
                            title="RAM MOUNTS"
                          >
                            RAM MOUNTS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/reso"
                            class="mtc-link"
                            title="RESO"
                          >
                            RESO
                          </a>
                        </li>
                        <li>
                          <a
                            href="/revit"
                            class="mtc-link"
                            title="REVIT"
                          >
                            REVIT
                          </a>
                        </li>
                        <li>
                          <a
                            href="/red-bull-spect"
                            class="mtc-link"
                            title="RED BULL SPECT"
                          >
                            RED BULL SPECT
                          </a>
                        </li>
                        <li>
                          <a
                            href="/rhinowalk"
                            class="mtc-link"
                            title="RHINOWALK"
                          >
                            RHINOWALK
                          </a>
                        </li>
                        <li>
                          <a
                            href="/rok-straps"
                            class="mtc-link"
                            title="ROK STRAPS"
                          >
                            ROK STRAPS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/sas-tec"
                            class="mtc-link"
                            title="SAS-TEC"
                          >
                            SAS-TEC
                          </a>
                        </li>
                        <li>
                          <a
                            href="/sc-project"
                            class="mtc-link"
                            title="SC PROJECT"
                          >
                            SC PROJECT
                          </a>
                        </li>
                        <li>
                          <a
                            href="/shoei"
                            class="mtc-link"
                            title="SHOEI"
                          >
                            SHOEI
                          </a>
                        </li>
                        <li>
                          <a
                            href="/sp-connect"
                            class="mtc-link"
                            title="SP CONNECT"
                          >
                            SP CONNECT
                          </a>
                        </li>
                        <li>
                          <a
                            href="/sw-motech"
                            class="mtc-link"
                            title="SW MOTECH"
                          >
                            SW MOTECH
                          </a>
                        </li>
                        <li>
                          <a
                            href="/cascos-schuberth"
                            class="mtc-link"
                            title="SCHUBERTH"
                          >
                            SCHUBERTH
                          </a>
                        </li>
                        <li>
                          <a
                            href="/scorpion"
                            class="mtc-link"
                            title="SCORPION"
                          >
                            SCORPION
                          </a>
                        </li>
                        <li>
                          <a
                            href="/sena"
                            class="mtc-link"
                            title="SENA"
                          >
                            SENA
                          </a>
                        </li>
                        <li>
                          <a
                            href="/shad"
                            class="mtc-link"
                            title="SHAD"
                          >
                            SHAD
                          </a>
                        </li>
                        <li>
                          <a
                            href="/shaft"
                            class="mtc-link"
                            title="SHAFT"
                          >
                            SHAFT
                          </a>
                        </li>
                        <li>
                          <a
                            href="/shark"
                            class="mtc-link"
                            title="SHARK"
                          >
                            SHARK
                          </a>
                        </li>
                        <li>
                          <a
                            href="/shima"
                            class="mtc-link"
                            title="SHIMA"
                          >
                            SHIMA
                          </a>
                        </li>
                        <li>
                          <a
                            href="/shinko"
                            class="mtc-link"
                            title="SHINKO"
                          >
                            SHINKO
                          </a>
                        </li>
                        <li>
                          <a
                            href="/sin-marca"
                            class="mtc-link"
                            title="SIN MARCA"
                          >
                            SIN MARCA
                          </a>
                        </li>
                        <li>
                          <a
                            href="/spidi"
                            class="mtc-link"
                            title="SPIDI"
                          >
                            SPIDI
                          </a>
                        </li>
                        <li>
                          <a
                            href="/sumomoto"
                            class="mtc-link"
                            title="SUMOMOTO"
                          >
                            SUMOMOTO
                          </a>
                        </li>
                        <li>
                          <a
                            href="/tcx"
                            class="mtc-link"
                            title="TCX"
                          >
                            TCX
                          </a>
                        </li>
                        <li>
                          <a
                            href="/taurus"
                            class="mtc-link"
                            title="TAURUS"
                          >
                            TAURUS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/thor"
                            class="mtc-link"
                            title="THOR"
                          >
                            THOR
                          </a>
                        </li>
                        <li>
                          <a
                            href="/torc"
                            class="mtc-link"
                            title="TORC"
                          >
                            TORC
                          </a>
                        </li>
                        <li>
                          <a
                            href="/tripmachine"
                            class="mtc-link"
                            title="TRIPMACHINE"
                          >
                            TRIPMACHINE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/tsubaki"
                            class="mtc-link"
                            title="TSUBAKI"
                          >
                            TSUBAKI
                          </a>
                        </li>
                        <li>
                          <a
                            href="/vulcanet"
                            class="mtc-link"
                            title="VULCANET"
                          >
                            VULCANET
                          </a>
                        </li>
                        <li>
                          <a
                            href="/wagen-covers"
                            class="mtc-link"
                            title="WAGEN COVERS"
                          >
                            WAGEN COVERS
                          </a>
                        </li>
                        <li>
                          <a
                            href="/wanderwarm"
                            class="mtc-link"
                            title="WANDERWARM"
                          >
                            WANDERWARM
                          </a>
                        </li>
                        <li>
                          <a
                            href="/x-tire"
                            class="mtc-link"
                            title="X-TIRE"
                          >
                            X-TIRE
                          </a>
                        </li>
                        <li>
                          <a
                            href="/ycf"
                            class="mtc-link"
                            title="YCF"
                          >
                            YCF
                          </a>
                        </li>
                      </ul>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col s12 submenu__extra-nav">
                <div class="extra__container" />
              </div>
            </div>
          </div>
        </div>
      </li>
    </ul>
  </VMenu>


  <!-- /marcas -->
</template>

<style lang="scss">

#nav ul li {
  display: inline-block;
  line-height: inherit;
}
#nav ul.u0 #ni_2529_marcas ul.u1 a {
  line-height: 5px;
}

#nav .submenu__outstanding-list ul li {
  margin-bottom: 3px;
  max-width: 90%;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

#nav .submenu__content li .mtc-link, .submenu__content li > a {
  display: flex;
  flex-wrap: wrap;
  font-size: 10px;
  font-weight: 500;
  line-height: 16px;
  text-transform: uppercase;
}

.submenu__outstanding-showcase {
  display: flex;
  flex-wrap: wrap;
  justify-content: space-between;
  margin-bottom: 10px;
}

ul.u0 #ni_2529_marcas ul.u1 a {
  line-height: 5px;
}
.submenu__outstanding-showcase.brands .mtc-link, .submenu__outstanding-showcase.brands a {
  flex: 1 1 auto;
}

.submenu__outstanding-showcase .mtc-link, .submenu__outstanding-showcase a {
  display: block;
  flex: 0 1 18.6666%;
  margin-bottom: 8px;
}

.submenu__outstanding-showcase:has(>:last-child:nth-child(6)) a, .submenu__outstanding-showcase:has(>:last-child:nth-child(6)) span.mtc-link {
  flex: 1 1 16.66%;
}



.submenu__outstanding-showcase .mtc-link img, .submenu__outstanding-showcase a img {
  display: block;
  height: auto;
  margin: 0 auto 3px;
  width: 80px;
}

.submenu__outstanding-showcase .mtc-link span, .submenu__outstanding-showcase a span {
  color: #000;
  display: block;
  font-size: 10px;
  font-weight: 500;
  margin: 0;
  text-transform: uppercase;
}

.submenu__outstanding-icon {
  display: inline-block;
  text-align: center;
}
.submenu__outstanding-showcase {
  display: flex;
  flex-wrap: wrap;
  justify-content: space-between;
  margin-bottom: 10px;
}

.submenu__outstanding-showcase .mtc-link, .submenu__outstanding-showcase a {
  display: block;
  flex: 0 1 18.6666%;
  margin-bottom: 8px;
}


.submenu__outstanding-list ul {
  -webkit-columns: 100px 6;
  -moz-columns: 100px 6;
  columns: 100px 6;
}
.submenu__outstanding-list ul li {
  margin-bottom: 3px;
  max-width: 90%;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.submenu__content li .mtc-link, .submenu__content li > a {
  display: flex;
  flex-wrap: wrap;
  font-size: 10px;
  font-weight: 500;
  line-height: 16px;
  text-transform: uppercase;
}

</style>
